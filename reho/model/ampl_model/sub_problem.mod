...
param lca_kpi_2{k in Lca_kpi, u in Units} default 0; # Construction
...
lca_tot[k] = sum{u in Units} lca_units[k, u] + sum{l in ResourceBalances, u in UnitsOfLayer[l]} lca_op[k, l, u] + sum{l in ResourceBalances} lca_res[k, l];

subject to LCA_total_house{k in Lca_kpi, h in House}:
lca_tot_house[k, h] = sum{u in UnitsOfHouse[h]} lca_units[k, u] + sum{l in ResourceBalances, u in UnitsOfLayer[l] inter UnitsOfHouse[h]} lca_op[k, l, u] + sum{l in ResourceBalances,p in PeriodStandard,t in Time[p]} (lca_kpi_supply[k,l,p,t]*Grid_supply[l,h,p,t]-lca_kpi_demand[k,l,p,t]*Grid_demand[l,h,p,t]) *dp[p]*dt[p];
